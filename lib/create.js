...
const create = function create(...args) {
  ...
  if (process.env.TOKEN) {
    spawn({cwd: projectdirectory}, 'curl', '-i', '-H', `'Authorization: token ${process.env.TOKEN}'`, '-d', `'{"name": "${REPO}"}'`, 'https://api.github.com/user/repos');
    ...
    spawn({cwd: projectdirectory}, 'git', 'commit', '-am', '\'initial commit\'');
